...
public class Main {
    ...
    /** This is the number of seconds between dumping images to a usb stick, this is taken from the vision parameters **/
	public static int seconds_between_img_dumps;
	/** This is the current number of seconds, this resets every time it is over the seconds_between_img_dumps **/
    public static long current_time_seconds;
    /** The directory that images are dumped to **/
    public static String outputPath;
    /** The data format, it is yyyy.MM.dd-hh-mm-ss , this is used when dumping images to mark the time**/
	...
	public static class VisionParams {
		...
		int cameraSelect;
		/** The threshold to detect one large cube as 2 cubes, this is a value between 0 and 1 **/
		double aspectRatioThresh;
		/** The minimum area that a target can have and still be recognized **/
		double minArea;
		/** How important it is for a target to be close to the center of the image, this will change depending on how well we can turn **/
		...
		int height;
		/** The size to resize the image from the camera to, this is just the width and the height values **/
		Size sz;
	...
	public static class VisionData {
		...
		public static class Target {
			...
			double yCentreNorm;
			/** A value between 0 and 1 representing the percentage of the image the target takes up **/
		...
		/** The target that is the most appealing, how it is chosen depends on the distToCenterImportance value in the vision parameters **/
	...
	private static void loadVisionParams() {
		...
		try {
			//The file input stream that contains the vision parameters data from visionParams.properties
			FileInputStream in = new FileInputStream("visionParams.properties");
			//Tells the properties to use the file input stream
			properties.load(in);
			//Sets the selected camera to the selected camera in the properties file
			...
			//Sets the minimum saturation to the minimum saturation in the properties file
			visionParams.minSaturation = Integer.valueOf(properties.getProperty("minSaturation"));
			//Sets the maximum saturation to the maximum saturation in the properties file
			visionParams.maxSaturation = Integer.valueOf(properties.getProperty("maxSaturation"));
			//Sets the minimum value to the minimum value in the properties file
			visionParams.minValue = Integer.valueOf(properties.getProperty("minValue"));
			//Sets the maximum value to the maximum value in the properties file
			...
			//Sets the erode dilate iterations to the erode dilation iterations in the properties file
			visionParams.erodeDilateIterations = Integer.valueOf(properties.getProperty("erodeDilateIterations"));
            //Sets the aspect ratio threshold to the aspect ratio threshold in the properties file
			visionParams.aspectRatioThresh = Double.valueOf(properties.getProperty("aspectRatioThresh"));
			//Sets the distance to center importance to the distance to center importance in the properties file
			visionParams.distToCentreImportance = Double.valueOf(properties.getProperty("distToCentreImportance"));
            //Sets the output path for dumping images to the image dump path in the properties file
			outputPath = properties.getProperty("imgDumpPath");
			//Sets the seconds between image dumps to the seconds between image dumps in the properties file
            seconds_between_img_dumps = Integer.valueOf(properties.getProperty("imgDumpWait"));
            //Sets the image file path to the image file path in the properties file, this is only used if the selected camera is -1
	...
	public static void saveVisionParams() {
		...
		Properties properties = new Properties();
		try {
			//Sets the camera select property in the file to the camera select value
			properties.setProperty("CameraSelect", String.valueOf(visionParams.cameraSelect));
			//Sets the minimum hue property in the file to the minimum hue value
			properties.setProperty("minHue", String.valueOf(visionParams.minHue));
			//Sets the maximum hue property in the file to the maximum hue value
			properties.setProperty("maxHue", String.valueOf(visionParams.maxHue));
			//Sets the minimum saturation property in the file to the minimum saturation value
			properties.setProperty("minSaturation", String.valueOf(visionParams.minSaturation));
			//Sets the maximum saturation property in the file to the maximum saturation value
			properties.setProperty("maxSaturation", String.valueOf(visionParams.maxSaturation));
			//Sets the minimum value property in the file to the minimum value value
			properties.setProperty("minValue", String.valueOf(visionParams.minValue));
			//Sets the maximum value property in the file to the maximum value value
			properties.setProperty("maxValue", String.valueOf(visionParams.maxValue));
			//Sets the erode dilate iterations property in the file to the erode dilate iterations value
			properties.setProperty("erodeDilateIterations", String.valueOf(visionParams.erodeDilateIterations));
			//Sets the minimum area property in the file to the minimum area value
			properties.setProperty("minArea", String.valueOf(visionParams.minArea));
			//Sets the aspect ratio threshold property in the file to the aspect ratio threshold value
			properties.setProperty("aspectRatioThresh", String.valueOf(visionParams.aspectRatioThresh));
			//Sets the distance to center importance property in the file to the distance to center importance value
			...
			//Sets the image dumping interval property in the file to the image dumping interval value
			properties.setProperty("imgDumpWait", String.valueOf(seconds_between_img_dumps));
			//Sets the image dumping path property in the file to the image dumping path value
        ...
        while (true) {
            ...
            if (use_GUI) {
                try {
                    ...
                    // the window failed
                	//Update the raw image gui
                    guiRawImg.updateImage(matToBufferedImage(rawOutputImg));
                    //Update the processed image gui
            ...
            //Calculate the time spent processing the image
